...
from app.models import db, User, BenchmarkResult, AuditLog, GroupChat, GroupMember, Message, UserSessionKey, UserChatPreference, UserReport
...
def get_chat_history():
    ...
    # Auto-purge expired disappearing messages
    try:
        now = datetime.utcnow()
        expired_msgs = Message.query.filter(Message.expires_at.isnot(None), Message.expires_at < now).all()
        if expired_msgs:
            for em in expired_msgs:
...
def set_chat_lock():
    """Sets, updates, or removes passcode lock on a chat conversation."""
    if 'user_id' not in session:
        return jsonify({'error': 'Unauthorized.'}), 401
    current_user_id = session['user_id']
    data = request.get_json() or {}
    ...
    action = data.get('action', 'set') # 'set', 'change', 'disable'
    ...
    # If already locked, verify current passcode before allowing change or removal
    if pref.is_locked and pref.lock_pin_hash:
        if not old_pin:
            return jsonify({'error': 'Current passcode is required.'}), 400
        old_hash = hashlib.sha256(f"pqc-lock-{old_pin}".encode('utf-8')).hexdigest()
        if old_hash != pref.lock_pin_hash:
            return jsonify({'error': 'Incorrect current passcode. Access denied.'}), 400
    ...
    return jsonify({
        ...
        'message': 'Chat lock removed.' if not pref.is_locked else 'Chat passcode set successfully.',
...
def report_user():
    """Submits a security or misuse report against a user and applies protective actions."""
    if 'user_id' not in session:
        return jsonify({'error': 'Unauthorized.'}), 401
    current_user_id = session['user_id']
    data = request.get_json() or {}
    ...
    reason = data.get('reason', 'Misuse / Abuse').strip()
    details = data.get('details', '').strip()
    message_id = data.get('message_id')
    block_user = data.get('block_user', False)
    ...
    # Log high-risk audit
    reporter_name = session.get('username', 'User')
    audit = AuditLog(
        user_id=current_user_id,
        action="USER_REPORTED_MISUSE",
        algorithm="N/A",
        mode="SYSTEM",
        result="SUCCESS",
        risk_level="HIGH",
        details=f"User '{reported_user.username}' was reported for '{reason}' by '{reporter_name}'. Details: {details[:150]}",
        ip_address=request.remote_addr
    )
    db.session.add(audit)
    
    # Apply protective block if requested
    is_blocked = False
    if block_user:
        pref = _get_or_create_pref(current_user_id, peer_id=reported_user_id)
        if pref:
    ...
    try:
        from app import socketio
        socketio.emit('security_monitor_update', {
            'type': 'USER_REPORT_LOGGED',
    ...
    return jsonify({
        ...
        'report_id': report.id,
        'blocked': is_blocked,
        'message': f"Report successfully submitted for '{reported_user.username}'. Protective security active."
    }), 201

@api_bp.route('/chat/my_reports', methods=['GET'])
def get_my_reports():
    """Returns all reports submitted by the current logged-in user."""
    if 'user_id' not in session:
        return jsonify({'error': 'Unauthorized.'}), 401
    current_user_id = session['user_id']
    reports = UserReport.query.filter_by(reporter_id=current_user_id)\
        .order_by(UserReport.created_at.desc()).limit(100).all()
    return jsonify([r.to_dict() for r in reports]), 200

@api_bp.route('/chat/preferences/disappearing', methods=['GET', 'POST'])
def manage_disappearing_timer():
    """Gets or sets the disappearing messages expiration duration in days."""
    if 'user_id' not in session:
        return jsonify({'error': 'Unauthorized.'}), 401
    current_user_id = session['user_id']
    ...
    if request.method == 'GET':
        ...
        if not pref:
            return jsonify({'error': 'peer_id or group_id required.'}), 400
        return jsonify({'disappearing_days': pref.disappearing_days or 0}), 200
    ...
    pref.disappearing_days = days
    pref.updated_at = db.func.now()
    
    # Sync with peer chat preference so both have matching expiration setting
    if peer_id:
        peer_pref = _get_or_create_pref(peer_id, peer_id=current_user_id)
        if peer_pref:
            peer_pref.disappearing_days = days
            peer_pref.updated_at = db.func.now()
            
    db.session.commit()
    
    # Broadcast to peer socket room
    try:
        from app import socketio
        socketio.emit('disappearing_timer_updated', {
    ...
    msg_str = f"Disappearing messages set to {days} days." if days > 0 else "Disappearing messages turned off."
    return jsonify({
        'disappearing_days': days,
        'message': msg_str
    }), 200
