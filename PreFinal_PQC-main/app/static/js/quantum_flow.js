...
const QuantumFlow = (() => {
    ...
    function startLoginFlow(user, callback) {
        ...
        const portal = document.getElementById('quantum-login-portal');
        if (!portal || typeof THREE === 'undefined') {
            if (portal) {
                portal.classList.remove('active');
                portal.style.display = 'none';
            }
        ...
        // Safety fallback timeout: guaranteed to transition within 2.6s even if rendering fails
        const safetyTimeout = setTimeout(() => {
            if (isRunning) {
                console.warn("QuantumFlow safety timeout triggered, finalizing transition.");
                finishFlow();
            }
        }, 2600);
        ...
        try {
            ...
            clearTimeout(safetyTimeout);
            console.warn("QuantumFlow WebGL init failed, bypassing animation:", e);
            if (portal) {
                portal.classList.remove('active');
                portal.style.display = 'none';
            }
